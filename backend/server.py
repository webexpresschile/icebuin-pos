...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> User:
    try:
        ...
        user['created_at'] = normalize_datetime(user['created_at'])
...
async def get_products(current_user: User = Depends(get_current_user)):
    ...
    for product in products:
        product['created_at'] = normalize_datetime(product['created_at'])
    return products
...
async def update_product(product_id: str, product_data: ProductUpdate, current_user: User = Depends(get_current_user)):
    ...
    updated_product['created_at'] = normalize_datetime(updated_product['created_at'])
...
async def get_sales(current_user: User = Depends(get_current_user)):
    ...
    for sale in sales:
        sale['created_at'] = normalize_datetime(sale['created_at'])
    return sales

@api_router.get("/sales/recent")
async def get_recent_sales(limit: int = 10, current_user: User = Depends(get_current_user)):
    """Get recent sales for dashboard"""
    sales = await db.sales.find({}, {"_id": 0}).sort("created_at", -1).limit(limit).to_list(limit)
    for sale in sales:
        sale['created_at'] = normalize_datetime(sale['created_at'])
    return sales
...
async def get_sales_report(start_date: str, end_date: str, current_user: User = Depends(get_current_user)):
    try:
        ...
        end = normalize_datetime(end_date).replace(hour=23, minute=59, second=59)
    except:
    ...
    filtered_sales = []
    for sale in sales:
        sale_date = normalize_datetime(sale['created_at'])
